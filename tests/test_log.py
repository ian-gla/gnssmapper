...
class TestReadCSV(unittest.TestCase):
    ...
    def test_read_csv_(self) -> None:
        ...
        raw_expected = pd.DataFrame(
            ...
            convert_floating=False, convert_boolean=False, convert_string=False
        )
...
class TestProcessRaw(unittest.TestCase):
    ...
    def test_galileo_ambiguity(self) -> None:
        ...
        np.testing.assert_array_equal(log.galileo_ambiguity(testdata), expected)
    ...
    def test_period_start_time(self) -> None:
        ...
        state = 9
        constellation = "G"
        expected = tm.gpsweek_to_gps(self.rx.week, pd.Series([0]), pd.Series([0]))
        pt.assert_series_equal(
            log.period_start_time(
    ...
    def test_rx(self) -> None:
        ...
        pt.assert_extension_array_equal(
            output.rx.array, expected.array, check_exact=True
        )

    def test_tx(self) -> None:
        output = log.process_raw(self.input)
        pt.assert_extension_array_equal(
            output.tx.array, self.tx_gps.array, check_exact=True
        )
    ...
    def test_tx_prior_week(self) -> None:
        ...
        tx_ = pd.DataFrame(
            {"week": [1999], "day": [6], "time": [cn.nanos_in_day - 1 * 10**7]}
        )
        ...
        pt.assert_extension_array_equal(output.tx.array, tx_gps.array, check_exact=True)
...
class TestNA(unittest.TestCase):
    ...
    def test_joining(self) -> None:
        ...
        self.assertWarnsRegex(
            UserWarning,
            "37 observations discarded without matching fix.",
