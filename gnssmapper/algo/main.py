...
def fit_data(data: pd.DataFrame, **kwargs) -> pd.DataFrame:
    ...
    params = [
        fit(height=data[i], ss=data["Cn0DbHz"], **kwargs)[-1, :, :] for i in index
    ]
    heights = np.array([_heights(p) for p in params])
    return pd.DataFrame(
        heights, index=index, columns=["lower_bound", "mid_point", "upper_bound"]
    )
...
def fit_edge(
    height: np.array,
    ss: np.array,
    starting_params: np.array = np.array([40, 0.1, 40, 10]),
) -> np.array:
...
def fit(
    ...
    online_params: dict = {
        "ss_lr": [0.0002, 0.01, 0.1, 0.0002],
        "height_lr": [0.00001, 0.01, 0.1, 0.00001],
        "batch_size": 100,
    },
) -> np.array:
    ...
    starting_params: list, optional
        starting parameters [ss_params,height_params] for the algorithm,
        otherwise generated automatically. Each param is a tuple (a,b,c,d)
    ...
    ss_model = fpl(
        ...
        initial_param=starting_params[0],
    )
    height_model = fpl(
        lr=model_args["height_lr"],
        batch_size=model_args["batch_size"],
        initial_param=starting_params[1],
    )
    ...
    for pos in range(0, total_size, batch_size):
        ...
        param_stream = (
            height_model.fit_online(h, y) if online else height_model.fit_offline(h, y)
        )
        h_param.extend(param_stream)
        # fit the ss model
        y = h > height_model.param[2]
        param_stream = (
            ss_model.fit_online(s, y) if online else ss_model.fit_offline(s, y)
        )
