...
def read_gnsslogger(filepath: str) -> ReceiverPoints:
    ...
    points = join_receiver_position(gnss_obs, gnss_fix)
...
def read_csv_(filepath: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    with open(filepath) as f:
        ...
        gnss_raw = pd.read_csv(StringIO("\n".join(raw))).convert_dtypes(
    ...
    with open(filepath) as f:
        ...
        gnss_fix = pd.read_csv(StringIO("\n".join(fix))).convert_dtypes(
            convert_string=False,
            convert_integer=True,
            convert_boolean=False,
            convert_floating=False,
...
def process_raw(gnss_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    tx = (
        ...
        + period_start_time(rx, gnss_raw["State"], constellation)
    ...
    tx = (
        tx
        + cm.constants.leap_seconds(cm.time.gps_to_utc(tx)).where(
            constellation == "R", 0
        )
        * 10**9
    )
    ...
    check_ = rx > tx
    if not check_.all():
        warnings.warn("rx less than tx, corrected assuming due to different gps weeks")
...
def galileo_ambiguity(x: np.array) -> np.array:
    ...
    """
    return cm.constants.nanos_in_period["E"] * (x // cm.constants.nanos_in_period["E"])
...
def join_receiver_position(
    gnss_obs: pd.DataFrame, gnss_fix: pd.DataFrame
) -> ReceiverPoints:
    ...
    """
    clean_fix = (
        gnss_fix[["Longitude", "Latitude", "Altitude", "(UTC)TimeInMs"]]
        .dropna()
        .sort_values("(UTC)TimeInMs")
    )
